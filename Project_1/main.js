...
function q1() {
	...
	$.ajax({
		...
		statusCode: {
			404: function (xhr) {
				$("#names-q1").append("<li>해당 플레이어를 찾을 수 없습니다. (404에러)</li>");
				$("#loading").addClass("hidden");
				const $btn = $("#search-button");
				$btn.prop("disabled", false).text($btn.data("orig") || "검색");
			},
	...
	function showListAnimated(list) {
		...
		$ul.empty();
		let delay = 0;
		const bulletImgUrl = "image/pngegg.png";
...
$(function () {
	// 팬 이미지 경로
	const panNormal = "image/pen.png";
	const panLoading = "image/pen_normal.png";
	...
	$("#search-button").on("click", function () {
		$(this).prop("disabled", true).data("orig", $(this).text()).text("Loading...");
		$("#loading").removeClass("hidden");
		$("#pan-loading-img").attr("src", panLoading);
		startLoadingAnim();
		q1();
	});
	...
	const parachuteImgUrl = "image/pngimg.com - parachute_PNG19426.png";
	let killCount = 0;
	let goPlayShown = false;
	function showExplosion(x, y, size = 120) {
		const explosion = document.createElement("img");
		explosion.src = "image/ChatGPT Image 2025년 8월 12일 오후 08_19_01.png";
		explosion.alt = "폭발";
	...
	BULLET_IMG.src = "image/pngegg.png";
	function loop() {
